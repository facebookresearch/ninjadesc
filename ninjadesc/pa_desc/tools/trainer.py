...
from ninjadesc.pa_desc.engine.joint_module import JointPADescLemuriaNetModule
...
@hydra.main(config_path="../config", config_name="config_joint")
...
def train(cfg: DictConfig) -> LocalTrainResult:
    ...
    # Initialize Model
    model = JointPADescLemuriaNetModule(cfg)

    ####
    # Initialising datasets
    ####
    # Initialise Patch Train Dataset
    train_patch_transform = compose_transforms_from_hydra(cfg.data.train.tfs)
    train_patch_dataset = instantiate(
        cfg.data.train.ds, train=True, transform=train_patch_transform
    )
    train_patch_dataloader = instantiate(
        cfg.data.train.dl.patch, train_patch_dataset, shuffle=True
    )
    ...
    # Initialise Image (Recon) Train Dataset
    train_recon_dataset = MegaDepthDataset(
        h5_dir=h5_dir,
        mode="train",
        kpt_type=kpt_type,
        num_samples=50000,
    )
    train_recon_dataloader = instantiate(
        cfg.data.train.dl.recon, train_recon_dataset, shuffle=True
    )
    ...
    _ = trainer.fit(
        model,
        train_dataloaders={
            "patch": train_patch_dataloader,
            "recon": train_recon_dataloader,
        },
