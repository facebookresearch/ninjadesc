...
class JointPADescLemuriaNetModule(pl.LightningModule):
    def __init__(self, cfg: DictConfig):
        ...
        pa_desc_model = torch.jit.load(download_weights(PA_DESC_MODEL))
        self.sosnet = pa_desc_model.sosnet
        ...
        state_dict = torch.load(download_weights(RECON_MODEL))
    ...
    @staticmethod
    def extract_descs_from_sos_map(sos_map: torch.Tensor, desc_model) -> torch.Tensor:
        # TODO: Now the dims are hardcoded as B*D*H*W, could consider extra line of assert here to check, or using named dims
        (batch_size, dim_desc, h, w) = sos_map.shape

        # Locate where kpoints are
        kpt_locs = sos_map.sum(1).bool().reshape(-1)
        sos_map = sos_map.permute(0, 2, 3, 1).reshape(-1, 128)

        # SOSNet -> our descriptors on only those locations
        sos_map[kpt_locs] = desc_model(sos_map[kpt_locs])
        sos_map = sos_map.reshape(batch_size, h, w, dim_desc).permute(0, 3, 1, 2)

        return sos_map

    def forward(
        self, patches: torch.Tensor, feature_image: torch.Tensor, is_desc: bool = True
    ):

        # Forward pass through UNet to obtain predicted RGB
        im_pred = self.recon(
            self.extract_descs_from_sos_map(
                sos_map=feature_image, desc_model=self.privacy
            )
        )
        ...
        if is_desc:
            ...
            with torch.no_grad():  # gradients not required for SOSNet weights
                descriptor_a = self.sosnet(patches["patches"][:, 0])
                descriptor_b = self.sosnet(patches["patches"][:, 1])
    ...
    def validation_or_test_step(
        ...
        **kwargs,
        ...
        desc_a = self.privacy(self.sosnet((batch["patches"][:, 0])))
        desc_b = self.privacy(self.sosnet((batch["patches"][:, 1])))
